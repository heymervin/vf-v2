...
/**
 * runDailyBrief — emails a morning digest to each venue owner.
 *
 * Driven by Vercel Cron (see vercel.json: /api/cron/daily-brief at 07:00 UTC).
 *
 ...
 * an hour June–October). For the current single-venue TWM deployment the quick
 * fix is to run the cron at 0 6 * * * UTC. Fully correct per-timezone scheduling
 * requires a fan-out strategy — defer to post-GA.
 */
export async function runDailyBrief(): Promise<{
  venuesBriefed: number;
  results: unknown[];
}> {
  ...
  for (const venue of venues) {
    ...
    // which requires the service-role key — safe here (server-only).
    ...
    if (!ownerRes.data?.user_id) {
      // No owner membership found — skip gracefully.
      console.warn(
        `[daily-brief] No owner membership for venue ${venue.id} — skipping brief.`,
      );
      results.push({
        skipped: true,
        reason: "no-owner-membership",
        venueId: venue.id,
      });
      continue;
    }
    ...
    if (authError || !authUser?.user?.email) {
      ...
      results.push({
        skipped: true,
        reason: "owner-email-unresolvable",
        venueId: venue.id,
      });
      continue;
    }
    ...
    const upcomingEvents = (upcomingRes.data ?? []).map((ev) => {
      const wedding = Array.isArray(ev.weddings) ? ev.weddings[0] : ev.weddings;
      return {
        title: ev.title,
